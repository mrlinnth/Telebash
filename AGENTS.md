...
- `export TELEBASH_TOKEN="..."` sets the bot token for the current shell session.
...
- Use uppercase names for constants and environment-derived variables, such as `TG_TOKEN` or `TELEBASH_TOKEN`.
...
- Verifying token and favorites handling with `TELEBASH_TOKEN` and files in `~/.config/telebash/`.
...
Do not commit Telegram tokens or personal chat IDs. Treat exported secrets and `favorites.txt` as local-only data.
